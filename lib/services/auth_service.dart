...
class AuthService {
  ...
  static const String _primaryBaseUrl =
      'https://dev.advantagescm.com/LocalCRF/api';
  static const String _fallbackBaseUrl =
      'https://dev.advantagescm.com/LocalCRF/api'; // Fallback URL if primary fails
  static const String tokenKey = 'auth_token';
  static const String userDataKey = 'user_data';
  static const String baseUrlKey = 'base_url';
  static const String tlspvCredentialsKey =
      'tlspv_credentials'; // Key untuk menyimpan kredensial TLSPV
  static const String encryptionKey =
      'CRF_SECURE_KEY_2025'; // Key untuk enkripsi data
  ...
  Future<Map<String, dynamic>> getUserBranches(
      String username, String password, String noMeja,
    ...
    try {
      ...
      try {
        ...
        return {
          'success': false,
          'message':
              'Server returned invalid data: ${response.body.substring(0, min(100, response.body.length))}',
        };
      }
      ...
      if (response.statusCode == 200 && responseData['success'] == true) {
        return {
          'success': true,
          'message':
              responseData['message'] ?? 'Branches retrieved successfully',
  ...
  Future<Map<String, dynamic>> login(
      String username, String password, String noMeja,
      {String? selectedBranch, String? androidId, String? loginSebagai}) async {
    try {
      ...
      final normalizedLoginSebagai =
          (loginSebagai ?? '').toUpperCase() == 'TL' ? 'TL' : 'OPR';

      debugPrint(
          'Login attempt for user: $username, deviceId: $deviceId, selectedBranch: $selectedBranch, loginSebagai: $normalizedLoginSebagai');
      ...
      debugPrint('  - LoginSebagai: $normalizedLoginSebagai');
      ...
      final requestBody = {
        ...
        'LoginSebagai': normalizedLoginSebagai,
      };
      ...
      requestBody.forEach((key, value) {
        debugPrint(
            '  - Field "$key": ${value.runtimeType} = "${key == 'password' ? '***HIDDEN***' : value}"');
      });

      debugPrint('🔍 POWERSHELL COMPARISON:');
      debugPrint(
          'PowerShell worked with: {"username":"w-888","password":"Kepo@123","noMeja":"010101","AndroidId":"c74499a2bd1dec10","clientType":"Android","selectedBranch":"Cideng"}');
      debugPrint('Flutter sending: ${json.encode(requestBody)}');
      debugPrint(
          'Are they identical? ${json.encode(requestBody) == '{"username":"w-888","password":"Kepo@123","noMeja":"010101","AndroidId":"c74499a2bd1dec10","clientType":"Android","selectedBranch":"Cideng"}'}');
      ...
      if (response.statusCode != 200) {
        ...
        if (apiErrorMessage != null && apiErrorMessage.toString().isNotEmpty) {
          ...
          return {
            ...
            'errorType': errorType ??
                (apiErrorMessage.toString().toLowerCase().contains('androidid')
                    ? 'ANDROID_ID_ERROR'
                    : 'HTTP_ERROR')
          };
        } else {
      ...
      debugPrint(
          '  - Raw success value: $successValue (${successValue.runtimeType})');
      ...
      if (!isSuccess) {
        ...
        final isAndroidIdError =
            errorMessage.toString().toLowerCase().contains('androidid');

        debugPrint(
            '  - Error Type: ${isAndroidIdError ? 'ANDROID_ID_ERROR' : 'LOGIN_ERROR'}');
      ...
      if (selectedBranch != null && selectedBranch.isNotEmpty) {
        if (!userData.containsKey('branchName') ||
            userData['branchName'] == null ||
            userData['branchName'].toString().isEmpty) {
      ...
      if (groupIdRaw != null && groupIdRaw.toString().isNotEmpty) {
        ...
        if (!userData.containsKey('branchCode') ||
            ...
            userData['branchCode'].toString().isEmpty) {
      ...
      if (userRole == 'CRF_TL') {
        ...
        debugPrint(
            'Storing TLSPV credentials for QR code generation: ${credentialsSaved ? "SUCCESS" : "FAILED"}');

        // Verifikasi kredensial tersimpan
        final savedCreds = await getTLSPVCredentials();
        if (savedCreds != null) {
          debugPrint(
              'Verified saved TLSPV credentials: username=${savedCreds['username']}, hasPassword=${savedCreds['password'] != null}');
        } else {
      ...
      if (storedToken == null || storedToken.isEmpty) {
        debugPrint(
            'WARNING: Token storage verification failed! Stored token is null or empty.');
        ...
        if (retryToken == null || retryToken.isEmpty) {
          debugPrint(
              'ERROR: Token storage retry failed! Token still null or empty.');
          return {
            'success': false,
  ...
  Future<String?> getToken() async {
    try {
      ...
      if (token != null && token.isNotEmpty) {
        final parts = token.split('.');
        final displayPart = token.length > 10
            ? "${token.substring(0, 5)}...${token.substring(token.length - 5)}"
            : token;
        debugPrint(
            'Retrieved token: $displayPart (length: ${token.length}, parts: ${parts.length})');
      } else {
    ...
    try {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          debugPrint(
              'Failed to refresh token with primary URL: ${response.statusCode}');
        }
      } catch (e) {
  ...
  Future<String?> getUserRole() async {
    try {
      ...
      print(
          'DEBUG getUserRole: roleID=${userData['roleID']}, role=${userData['role']}');
  ...
  Future<bool> hasRole(String requiredRole) async {
    try {
      ...
      print(
          'DEBUG hasRole: comparing userRole=$userRole with requiredRole=$requiredRole');
  ...
  Future<bool> hasAnyRole(List<String> requiredRoles) async {
    try {
      ...
      print(
          'DEBUG hasAnyRole: checking userRole=$normalizedUserRole against requiredRoles=$requiredRoles');
      return requiredRoles
          .any((role) => role.toUpperCase() == normalizedUserRole);
    } catch (e) {
  ...
  Future<bool> saveTLSPVCredentials(String username, String password) async {
    try {
      if (username.isEmpty || password.isEmpty) {
        debugPrint(
            'Cannot save TLSPV credentials: username or password is empty');
        return false;
      }

      debugPrint(
          'Saving TLSPV credentials: username=$username, passwordLength=${password.length}');
      ...
      if (!success) {
        debugPrint(
            'ERROR: Failed to save TLSPV credentials to SharedPreferences!');
        return false;
      }
      ...
      if (saved == null || saved.isEmpty) {
        debugPrint(
            'ERROR: Failed to verify TLSPV credentials in SharedPreferences!');
        return false;
      }
  ...
  Future<Map<String, dynamic>?> getTLSPVCredentials() async {
    try {
      ...
      try {
        ...
        debugPrint(
            'Error accessing SharedPreferences for TLSPV credentials: $e');
        return null;
      }
      ...
      debugPrint(
          'Retrieved TLSPV credentials string length: ${credentialsString.length}');
      ...
      debugPrint(
          'Successfully retrieved TLSPV credentials: username=$username, hasPassword=${password != null}');
      return credentials;
    } catch (e) {
  ...
  Map<String, dynamic>? decryptDataFromQR(String encryptedData) {
    try {
      ...
      // Ambil komponen
      if (!secureData.containsKey('data') ||
          !secureData.containsKey('timestamp') ||
          !secureData.containsKey('signature')) {
        debugPrint(
            'QR data missing required fields: data=${secureData.containsKey('data')}, timestamp=${secureData.containsKey('timestamp')}, signature=${secureData.containsKey('signature')}');
        return null;
      }
      ...
      debugPrint(
          'Decoded JSON string: ${jsonString.length > 100 ? "${jsonString.substring(0, 100)}..." : jsonString}');
      ...
      debugPrint(
          'Signature validation: ${validSignature ? "VALID" : "INVALID"}');
      debugPrint(
          'Time validation: ${validTime ? "VALID" : "EXPIRED"} (${(now - msgTime) / (60 * 1000)} minutes old)');
      ...
      if (validSignature && validTime) {
        ...
        if (decodedData.containsKey('compressed') &&
            decodedData['compressed'] == true) {
        ...
        } else if (decodedData['username'] == null ||
            decodedData['username'].toString().isEmpty) {
          debugPrint(
              'QR data username is null or empty: ${decodedData['username']}');
        } else {
        ...
        if (!decodedData.containsKey('password')) {
          debugPrint('QR data missing password key');
            ...
            decodedData['password'].toString().isEmpty) {
        ...
        if (!validSignature) {
          debugPrint(
              'QR data signature invalid: expected=$calculatedSignature, received=$receivedSignature');
        }
        if (!validTime) {
          debugPrint(
              'QR data expired: ${(now - msgTime) / (60 * 1000)} minutes old');
        }
        return null;
      }
    } catch (e) {
